...
def comp(s, _comp=str.maketrans("ATCG", "TAGC")):
...
def convert_reads(fq1s, fq2s, out_dir=None, out=sys.stdout):
    for fq1, fq2 in zip(fq1s.split(","), fq2s.split(",")):
        ...
        if already_interleaved:
            ...
            selected_iter = chain.from_iterable(zip(q1_iter, q2_iter))

        for read_i, (name, seq, _, qual) in enumerate(selected_iter):
            if name is None:
                continue
            convert_and_write_read(name, seq, qual, read_i % 2, out_fq1, out_fq2)
            if len(seq) < 80:
                lt80 += 1
...
def convert_and_write_read(name, seq, qual, read_i, out_fq1, out_fq2):
    name = name.rstrip("\r\n").split(" ")[0]
    if name[0] != "@":
        sys.stderr.write(
            """ERROR!!!!
    ...
    # ========================
    char_a, char_b = (FWD_FROM, FWD_TO) if read_i == 0 else (REV_FROM, REV_TO)
    out = out_fq1 if read_i == 0 else out_fq2
    # ========================
    # keep original sequence as name.
    name = " ".join((name, "YS:Z:" + seq + "\tYC:Z:" + char_a + char_b + "\n"))
...
class Bam(object):
    ...
    def __repr__(self):
        return "Bam({chr}:{start}:{read}".format(
            chr=self.chrom, start=self.pos, read=self.read
        )

    def __str__(self):
        return (
            "\t".join(str(getattr(self, s)) for s in self.__slots__[:11])
            + "\t"
            + "\t".join(self.other)
        )
    ...
    def cig_len(self):
        return sum(c[0] for c in self.cigs() if c[1] in ("M", "D", "N", "EQ", "X", "P"))
...
def bwa_mem(
    ...
    if is_newer_b(conv_fa, (conv_fa + ".amb", conv_fa + ".sa")):
        ...
        raise BWAMethException(
            "first run bwameth.py index %s OR bwameth.py index-mem2 %s OR make sure the modification time on the generated c2t files is newer than on the .fa file"
            % (fa, fa)
        )

    if rg is not None and not rg.startswith("@RG"):
...
def handle_header(line, out=sys.stdout):
    ...
    if toks[0].startswith("@PG"):
        # out.write("\t".join(toks) + "\n")
        toks = [
            '@PG\tID:bwa-meth\tPN:bwa-meth\tVN:%s\tCL:"%s"'
            % (__version__, " ".join(x.replace("\t", "\\t") for x in sys.argv))
        ]
    out.write("\t".join(toks) + "\n")
...
"""
    with tempfile.NamedTemporaryFile(delete=True) as rfh:
        ...
        rfh.flush()
        for d in reader(
            f"|Rscript {rfh.name} {a.regions} {' '.join(a.bams)}",
            header=False,
        ):
            print("\t".join(d))
...
def convert_fqs(fqs):
    script = __file__
    return f"{sys.executable} {script} c2t_fq {fqs[0]} {fqs[1] if len(fqs) > 1 else ','.join(['NA'] * len(fqs[0].split(',')))}"
...
def main():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="bwa-meth.py {}".format(__version__),
    )
    ...
    align_parser.add_argument("--reference", help="reference fasta")
    align_parser.add_argument("-t", "--threads", type=int, default=6)
    align_parser.add_argument(
        "--read-group",
        help="read-group to add to bam in the same format as to bwa: '@RG\\tID:foo\\tSM:bar'",
    )
    align_parser.add_argument(
        "--set-as-failed",
        help="flag alignments to this strand as not passing QC. Targetted BS-Seq libraries are often to a single strand...",
        default=None,
        choices=("f", "r"),
    )
    align_parser.add_argument(
        ...
        "--interleaved",
        action="store_true",
        help="fastq files have 4 lines of read1 followed by 4 lines of read2...",
    )
    align_parser.add_argument(
        "--do-not-penalize-chimeras",
        action="store_true",
        help="do not use the heuristic...",
    )
    align_parser.add_argument(
        "fastqs",
        nargs="+",
        help="bs-seq fastqs to align...",
    )
    align_parser.add_argument(
        "--config", type=str, choices=["bs-seq", "glori"], default="bs-seq"
    )
    ...
    global FWD_FROM, FWD_TO, REV_FROM, REV_TO
    if args.config == "bs-seq":
        FWD_FROM, FWD_TO = "C", "T"
        REV_FROM, REV_TO = "G", "A"
    elif args.config == "glori":
        FWD_FROM, FWD_TO = "T", "C"
        REV_FROM, REV_TO = "A", "G"
    else:
    ...
    if args.command == "convert":
        ...
        bwa_mem(
            args.reference,
            conv_fqs_cmd,
            " ".join(map(str, pass_through_args)),
